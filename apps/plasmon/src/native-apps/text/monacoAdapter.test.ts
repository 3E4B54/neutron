...
import { createOwnedEditorModel, editorLanguageForName, syncEditorModelValue } from "./editorModel.ts";
...
test("two live Monaco surfaces for one semantic document own distinct models", () => {
  ...
  const first = createOwnedEditorModel("node:shared-document", 41, create);
  const second = createOwnedEditorModel("node:shared-document", 42, create);
